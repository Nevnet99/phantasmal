...
3. Optionally set the version, or leave **draft** checked to skip publishing.
4. The workflow builds Linux (AppImage), macOS (dmg + zip), and Windows (NSIS), uploads
   them to a draft [GitHub Release](https://github.com/Nevnet99/phantasmal/releases), then
   **publishes** that release once every OS job finishes (unless you chose draft).
5. After publish, a follow-up job copies those installers into
